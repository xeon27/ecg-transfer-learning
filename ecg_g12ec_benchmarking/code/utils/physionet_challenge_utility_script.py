...
def load_challenge_data(filename):
    ...
    ###### For reading from .dat files
    # x = wfdb.rdsamp(filename.split('.dat')[0])
    # data = np.array(x[0]).T
    # print(data.shape)
    # header_file_path = "".join(filename.split('G12ECRes/records500/'))
    # new_file = header_file_path.replace('.dat', '.hea')
...
def import_key_data(path, icd_label_to_numerical_code, selected_ecg_ids):
    gender = []
    age = []
    labels = []
    ecg_filenames = []
    ...
    for subdir, dirs, files in sorted(os.walk(path)):
        for filename in files:
            ...
            if filepath.endswith(".mat") and filename.split('.mat')[0] in selected_ecg_ids:
            # if filepath.endswith(".dat") and filename.split('.dat')[0] in selected_ecg_ids:
                data, header_data = load_challenge_data(filepath)
                # all_data.append(data)
                selected_labels = []
                raw_labels = header_data[15][5:-1]
                for label in raw_labels.split(','):
                    if int(label) in icd_label_to_numerical_code.values():
            ...
            # pdb.set_trace()
    return gender, age, labels, ecg_filenames #, all_data
...
def make_undefined_class(labels, df_unscored):
    all_valid_labels = []
    for label in labels:
        all_labels_in_row = label.split(',')
        valid_labels = []
        for each_label in all_labels_in_row:
            if each_label in [str(i) for i in df_unscored.iloc[:, 1].tolist()]:
                valid_labels.append(each_label)
            else:
                valid_labels.append('undefined class')
        all_valid_labels.append(','.join(valid_labels))
    df_labels = pd.DataFrame(all_valid_labels)
    return df_labels
...
def onehot_encode(df_labels):
    ...
    return y, one_hot.classes_[0:], one_hot
...
def generate_validation_data(ecg_filenames, y, test_order_array):
    ...
    #X_train_gridsearch = X_train_gridsearch.reshape(ecg_filenames_train_gridsearch.shape[0], 1000, 12)
...
def generate_validation_data_with_demo_data(ecg_filenames, y, gender, age, test_order_array):
    ...
    #X_val = X_val.reshape(ecg_filenames_train_gridsearch.shape[0], 1000, 12)
...
def encoder_model():
    ...
    #input_layer = keras.layers.Input(shape=(1000, 12))
...
def compute_challenge_metric_for_opt(labels, outputs):
    classes = ['10370003', '111975006', '164889003', '164890007', '164909002', '164917005', '164934002', '164947007',
               '17338001',
               '251146004', '270492004', '284470004', '39732003', '426177001', '426627000', '426783006', '427084000',
               '427172004', '427393009', '445118002', '47665007', '59118001',
...
def plot_normalized_conf_matrix(y_pred, ecg_filenames, y, val_fold, threshold, snomedclasses, snomedabbr):
    conf_m = compute_modified_confusion_matrix(generate_validation_data(ecg_filenames, y, val_fold)[1],
                                               (y_pred > threshold) * 1)
...
def DetectRWithPanTompkins(signal, signal_freq):
    ...
    normalized = (integrated_squared_diff_y - min(integrated_squared_diff_y)) / (
                max(integrated_squared_diff_y) - min(integrated_squared_diff_y))
...
def plot_normalized_conf_matrix_rule(y_true, val_data, rb_pred, snomedclasses):
    df_cm = pd.DataFrame(compute_modified_confusion_matrix(y_true[val_data], rb_pred), columns=snomedclasses,
                         index=snomedclasses)
