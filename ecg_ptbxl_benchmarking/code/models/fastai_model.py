...
class fastai_model(ClassificationModel):
    ...
    def __init__(self, input):
        return predict(input.detach().cpu().clone().numpy())
    ...
    def __init__(self,name,n_classes,freq,outputfolder,input_shape,pretrained=False,input_size=2.5,input_channels=12,chunkify_train=False,chunkify_valid=True,bs=128,ps_head=0.5,lin_ftrs_head=[128],wd=1e-2,epochs=50,lr=1e-2,kernel_size=5,loss="binary_cross_entropy",pretrainedfolder=None,n_classes_pretrained=None,gradual_unfreezing=True,discriminative_lrs=True,epochs_finetuning=30,early_stopping=None,aggregate_fn="max",concat_train_val=False):
        ...
        self.epochs_gradual_unfreezing = epochs_finetuning
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        if(self.pretrainedfolder is None): #from scratch
            ...
            if(self.gradual_unfreezing):
                ...
                # print("Length of layer groups: ")
                # print(len(layer_groups))
                for n in range(len(layer_groups)):#[0]:
                    learn.freeze_to(-n-1)
    ...
    def _get_learner(self, X_train,y_train,X_val,y_val,num_classes=None):
        ...
        ds_train=TimeseriesDatasetCrops(df_train,self.input_size, random_crop=False,num_classes=self.num_classes,chunk_length=self.chunk_length_train if self.chunkify_train else 0,min_chunk_length=self.min_chunk_length,stride=self.stride_length_train,transforms=tfms_ptb_xl,annotation=False,col_lbl ="label",npy_data=X_train)
        ds_valid=TimeseriesDatasetCrops(df_valid,self.input_size,random_crop=False, num_classes=self.num_classes,chunk_length=self.chunk_length_valid if self.chunkify_valid else 0,min_chunk_length=self.min_chunk_length,stride=self.stride_length_valid,transforms=tfms_ptb_xl,annotation=False,col_lbl ="label",npy_data=X_val)
    
        db = DataBunch.create(ds_train,ds_valid,bs=self.bs) # 23-11-2021 REVERT
