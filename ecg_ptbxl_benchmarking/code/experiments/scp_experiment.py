...
class SCP_Experiment():
    ...
    def __init__(self, experiment_name, task, datafolder, outputfolder, models, sampling_frequency=100, min_samples=0, train_fold=[1, 2, 3, 4, 5, 6, 7, 8], val_fold=[9], test_fold=[10], folds_type='strat'):
        self.models = models
    ...
    def prepare(self,percentage_of_data=None, compsig = 'real'):
        # Load PTB-XL data
        self.data, self.raw_labels = utils.load_dataset(self.datafolder, self.sampling_frequency, per=percentage_of_data, compsig=compsig)
        ...
        self.X_test = self.data[self.labels.strat_fold.isin(self.test_fold)]
        self.y_test = self.Y[self.labels.strat_fold.isin(self.test_fold)]

        self.X_val = self.data[self.labels.strat_fold.isin(self.val_fold)]
        self.y_val = self.Y[self.labels.strat_fold.isin(self.val_fold)]

        self.X_train = self.data[self.labels.strat_fold.isin(self.train_fold)]
        self.y_train = self.Y[self.labels.strat_fold.isin(self.train_fold)]
        ...
        print("--------------------------------------------------")
        print("Saved the npy files at {} foldr and {}".format(self.outputfolder, self.experiment_name))
        print("--------------------------------------------------")
    ...
    def evaluate(self, n_bootstraping_samples=100, n_jobs=20, bootstrap_eval=False, dumped_bootstraps=True):
        ...
        def plot_calibration_curve(name, fig_index, probs, class_i):
            ...
            frac_of_pos, mean_pred_value = calibration_curve([i[class_i] for i in y_test], probs, n_bins=10)
        ...
        mlb = np.load(self.outputfolder+self.experiment_name+'/data/mlb.pkl', allow_pickle=True)
        ...
        for m in sorted(os.listdir(self.outputfolder+self.experiment_name+'/models')):
            ...
            # te_df, _ = pd.concat(pool.starmap(utils.generate_results, zip(test_samples, repeat(y_test), repeat(y_test_pred), repeat(thresholds))))
            # te_df_point, roc_point = utils.generate_results(range(len(y_test)), y_test, y_test_pred, thresholds)
            ...
            te_df_result.to_csv(rpath+'te_results_new.csv')
