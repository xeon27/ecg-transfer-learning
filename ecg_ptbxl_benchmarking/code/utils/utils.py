...
def evaluate_experiment(y_true, y_pred, thresholds=None):
    ...
    # results['weighted_auc'] = roc_auc_score(y_true, y_pred, average='weighted')
    # results['label_aucs'] = roc_auc_score(y_true, y_pred, average=None).tolist()
    
    df_result = pd.DataFrame(results, index=[0])
    # Return macro and per-class AUC
    return df_result, roc_auc_score(y_true, y_pred, average=None).tolist()
...
def choose_limited_data(df1, percentage=100):
    percentage_num = percentage//10
    choose_fold_numbers_from_original_folds = random.sample(list(range(1, 11)), percentage_num)
    df2 = df1[df1.strat_fold.apply(lambda x: x in choose_fold_numbers_from_original_folds)]
    return df2

def load_dataset(path, sampling_rate, release=False, per=None, compsig = None):
    if path.split('/')[-2] == 'ptbxl':
        # load and convert annotation data
        Y = pd.read_csv(path+'ptbxl_database.csv', index_col='ecg_id')
        Y.scp_codes = Y.scp_codes.apply(lambda x: ast.literal_eval(x))
        # Y = choose_limited_data(Y, per)
        # Load raw signal data
        X = load_raw_data_ptbxl(Y, sampling_rate, path, per)
        # For FFT experiment
        # X = load_raw_data_ptbxl_fft(Y, sampling_rate, path, compsig, per)
        print("Len of Y is %s"%str(len(Y)))
        print("Len of X is %s"%str(len(X)))

    #elif path.split('/')[-2] == 'G12ECRes':
    else:
        # load and convert annotation data
        Y = pd.read_csv(path+'g12ec_database.csv', index_col='ecg_id')
        Y.scp_codes = Y.scp_codes.apply(lambda x: ast.literal_eval(x))

        # Load raw signal data
        X = load_raw_data_g12ec(Y, sampling_rate, path)
        # For FFT experiment
        # X = load_raw_data_g12ec_fft(Y, sampling_rate, path, compsig)

    return X, Y


def load_raw_data_g12ec(df, sampling_rate, path):
    ...
    if sampling_rate == 100:
        if os.path.exists(path + 'raw100.npy'):
            ...
            # print(df.index)
            # print([path + 'records100/'+str(f) for f in tqdm(df.index)])
        ...
        if os.path.exists(path + 'raw500.npy'):
            ...
            data = [wfdb.rdsamp(path + str(f)) for f in tqdm(df.index)]
            data = np.array([signal for signal, meta in data])
            pickle.dump(data, open(path+'raw500.npy', 'wb'), protocol=4)
    return data

def load_raw_data_ptbxl(df, sampling_rate, path, per = None):
    if sampling_rate == 100:
        if os.path.exists(path + 'raw100.npy'):
            data = np.load(path+'raw100.npy', allow_pickle=True)
        else:
        ...
        if os.path.exists(path + 'raw500.npy') and per is None:
            data = np.load(path+'raw500.npy', allow_pickle=True)
        else:
            data = [wfdb.rdsamp(path+f) for f in tqdm(df.filename_hr)]
            data = np.array([signal for signal, meta in data])
            pickle.dump(data, open(path+'raw500_'+str(per)+'.npy', 'wb'), protocol=4)
    return data

############ For FFT experiment #############
def load_raw_data_g12ec_fft(df, sampling_rate, path, comp='real'):
    fname=f'raw{sampling_rate}_{comp}.npy'
    ...
    if sampling_rate == 100:
        if os.path.exists(path + fname):
            ...
            print(data.shape)
            data = np.array([np.fft.fft(d, axis=0) for d in data])
            print(data.shape)

            if comp == 'real':
                data = np.array([d.real for d in data])
            elif comp == 'imag':
                data = np.array([d.imag for d in data])
            elif comp == 'abs':
                data = np.array([np.abs(d) for d in data])
            pickle.dump(data, open(path+fname, 'wb'), protocol=4)
...
def load_raw_data_ptbxl_fft(df, sampling_rate, path, comp='real', per = None):
    fname=f'raw{sampling_rate}_{comp}.npy'
    ...
    if sampling_rate == 100:
        ...
        if os.path.exists(path + fname) and per is None:
            data = np.load(path+fname, allow_pickle=True)
        else:
            data = [wfdb.rdsamp(path+f) for f in tqdm(df.filename_hr)]
            data = np.array([signal for signal, meta in data])
